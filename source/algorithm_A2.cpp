        ...
        for(int i = 0 ;i<height;i++){
            for(int j = 0;j<width;j++){
                ...
                if(board.get_cell_color(i,j) != enemy_color){
                    ...
                    for(int k = 0;k<height;k++){
                        for(int h = 0;h<width;h++){
                            if(tmp.get_cell_color(k,h) != board.get_cell_color(k,h)){
                                P[i][j].score++;
                                if(board.get_orbs_num(k,h) == board.get_capacity(k,h)){
                                    P[i][j].score+=10;
                                }
                            }
                        }
                    }
                    if(i+1<height-1 &&  tmp.get_cell_color(i+1,j) == enemy_color){
                        P[i][j].score ++;
                    } 
                    if(i-1>=0 && tmp.get_cell_color(i-1,j) == enemy_color){
                        P[i][j].score++;
                    }
                    if(j+1<width-1 && tmp.get_cell_color(i,j+1) == enemy_color){
                           P[i][j].score++;
                        }
                    if(j-1>=0 && tmp.get_cell_color(i,j-1) == enemy_color){
                        P[i][j].score++;
                    }
                            ...
                            if(tmp.get_cell_color(k,h) != color){
                                ...
                                for(int a = 0;a<height;a++){
                                    for(int b = 0;b<width;b++){
                                        if(tmp1.get_cell_color(a,b) != tmp.get_cell_color(a,b)){
                                            P[i][j].point[k][h].eatnumber++;
                                        }
                                        if(tmp.get_cell_color(a,b) == color && tmp.get_orbs_num(a,b) == boardcost[a][b]-1){
                                            P[i][j].point[k][h].numbers ++;
                                        }
                                        if(tmp1.get_cell_color(i,j) == enemy_color){
                                            P[i][j].waste = true;
                                        }
                                    }
                                }
                    ...
                    for(int k = 0;k<height;k++){
                        for(int h = 0;h<width;h++){
                            max = P[i][j].point[k][h].eatnumber*10 + P[i][j].point[k][h].numbers;
                            if(P[i][j].punish < max){
                                P[i][j].punish = max;
                            }
                        }
